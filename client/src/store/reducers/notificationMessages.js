...
const ids = (state = [], action) => {
  switch (action.type) {
    case `${actions.LOAD_CONTACT_NOTIFICATION_MESSAGES}_REQUEST`:
    case `${actions.LOAD_CONTACT_NOTIFICATION_MESSAGES}_FAILED`: {
      return [];
    }

    case `${actions.LOAD_CONTACT_NOTIFICATION_MESSAGES}_SUCCESS`: {
  ...
  switch (action.type) {
    ...
    case `${actions.LOAD_CONTACT_NOTIFICATION_MESSAGES}_SUCCESS`: {
      return reducersUtils.updateEntities(
        state,
        action.payload.entities.notificationMessages,
      );
    }
